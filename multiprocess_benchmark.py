...
for run_index in range(TIMES_TO_RUN):
    ...
    with open("result" + str(run_index) + '.txt', 'w+') as out:
        cmd = ['./db_bench', '--benchmarks=fillrandom', '--use_existing_db=0', '--disable_auto_compactions=0', '--sync=0',
               '--db=./temp' + str(run_index), '--wal_dir=./temp' + str(run_index), '--num=625000', '--num_levels=6', '--key_size=16', '--value_size=400', '--block_size=8192', '--cache_size=6442450944',
               ...
               '--hard_rate_limit=3', '--rate_limit_delay_max_milliseconds=1000000', '--write_buffer_size=134217728', '--target_file_size_base=134217728',
               ...
               '--histogram=1', '--memtablerep=skip_list', '--bloom_bits=10', '--open_files=-1', '--max_background_compactions=16',
               '--max_write_buffer_number=8', '--max_background_flushes=7', '--level0_file_num_compaction_trigger=4', '--level0_stop_writes_trigger=20',
               '--level0_slowdown_writes_trigger=12', '--threads=16', '--allow_concurrent_memtable_write=true', '--disable_wal=1', '--seed=1606010580']
        proc = subprocess.Popen(cmd, cwd="./release", stdout=out, stderr=out)
